...
UploadBuffer::Page::Page(size_t sizeInBytes)
   ...
   m_d3d12Resource = Buffer::Create(Buffer::Desc::Upload(m_PageSize).InitialState(D3D12_RESOURCE_STATE_GENERIC_READ));

   m_d3d12Resource->SetName("Upload Buffer (Page)");

   m_GPUPtr = m_d3d12Resource->GetGPUVirtualAddress();
   m_CPUPtr = m_d3d12Resource->Map();
}

UploadBuffer::Page::~Page() {
   m_d3d12Resource->Unmap();
   m_CPUPtr = nullptr;
...
UploadBuffer::Allocation UploadBuffer::Page::Allocate(size_t sizeInBytes, size_t alignment) {
   ...
   allocation.uploadBuffer = m_d3d12Resource;
   allocation.uploadBufferOffset = m_Offset;
